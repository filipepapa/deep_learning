...
class Model1:
    ...
    def run_fit(self):
        ...
        self.y_pred = np.where(self.model.predict(self.g_, verbose=True) > 0.5, 1, 0)
...
g_boruta = TimeseriesGenerator(X_train_boruta, y_train, length=seqlen)
g_boruta_ = TimeseriesGenerator(X_test_boruta, y_test, length=seqlen)
...
model_2_pca.plot_acc_loss(model_2_pca.model_name)
#%%
# Running Model 2 PCA MinMax
monitor = 'loss'
numfeat = g_pca_minmax.data.shape[1] #scale minmax in pca

model_2_pca_minmax = Model2(g_pca_minmax,g_pca_minmax_,seqlen,numfeat,class_weight,model_name=f'model_2_pca_minmax_{monitor}')
model_2_pca_minmax.set_fit(es_monitor=monitor)

model_2_pca_minmax.create_model_best_hp(hbtuner_2_best)

history_2_pca_minmax, y_pred_2_pca_minmax = model_2_pca_minmax.run_fit()

model_2_pca_minmax.plot_acc_loss(model_2_pca_minmax.model_name,plot_val=True)

#%%
# Running Model 2 PCA Standard Scaler
monitor = 'loss'
numfeat = g_pca_std.data.shape[1] #scale in pca

model_2_pca_std = Model2(g_pca_std,g_pca_std_,seqlen,numfeat,class_weight,model_name=f'model_2_pca_std_{monitor}')
...
model_2_minmax.plot_acc_loss(model_2_minmax.model_name)
# %%
...
model_2_std.plot_acc_loss(model_2_std.model_name)
# %%
...
# %%
# Running Model 2 Boruta
monitor = 'loss'
numfeat = g_boruta.data.shape[1]

model_2_boruta = Model2(g_boruta,g_boruta_,seqlen,numfeat,class_weight,model_name=f'model_2_boruta_{monitor}')
model_2_boruta.set_fit(es_monitor=monitor)

model_2_boruta.create_model_best_hp(hbtuner_2_best)

history_2_boruta, y_pred_2_boruta = model_2_boruta.run_fit()
# %%
